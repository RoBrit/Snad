...
public class BlockSnad extends BlockFalling implements IMetaBlockSnad {
    ...
    public static final PropertyEnum<BlockSnad.EnumType> VARIANT = PropertyEnum.<BlockSnad.EnumType>create("variant", BlockSnad.EnumType.class);
...
//        super();
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(VARIANT, meta == 0 ? EnumType.SAND : EnumType.RED_SAND);
    }
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (blockAbove instanceof BlockReed || blockAbove instanceof BlockCactus) {
            ...
            while (isSameBlockType) {
                if (world.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1 + height, pos.getZ())).getBlock() != null) {
                    Block nextPlantBlock = world.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1 + height, pos.getZ())).getBlock();
                    ...
                    if (nextPlantBlock.getClass() == blockAbove.getClass()) {
                        if (height < 2) {
                            for (int growthAttempts = 0; growthAttempts < ConfigurationData.SPEED_INCREASE_VALUE; growthAttempts++) {
                                world.setBlockState(new BlockPos(pos.getX(), pos.getY() + 2 + height, pos.getZ()), nextPlantBlock.getDefaultState());
                            }
                        } else {
            ...
            world.setBlockState(new BlockPos(pos.getX(), pos.getY() + 2, pos.getZ()), blockAbove.getDefaultState());
        }
    }

    @Override
    public boolean canSustainPlant(IBlockAccess world, BlockPos pos, EnumFacing direction, IPlantable plantable) {
        Block plant = plantable.getPlant(world, new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ())).getBlock();
        ...
        switch (plantType) {
            ...
            case Water: {
                return world.getBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ())).getBlock().getMaterial() == Material.water
                        && world.getBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ())) == getDefaultState();
            }
            case Beach: {
                return (world.getBlockState(new BlockPos(pos.getX() - 1, pos.getY(), pos.getZ())).getBlock().getMaterial() == Material.water ||
                        world.getBlockState(new BlockPos(pos.getX() + 1, pos.getY(), pos.getZ())).getBlock().getMaterial() == Material.water ||
                        world.getBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ() - 1)).getBlock().getMaterial() == Material.water ||
                        world.getBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ() + 1)).getBlock().getMaterial() == Material.water);
            }
        }
